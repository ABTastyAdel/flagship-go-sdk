...
var defaultLogConsumer FlagshipLogConsumer
...
func SetLogger(logger FlagshipLogConsumer) {
	mutex.Lock()
...
func GetLogger(name string) FlagshipLogger {
...
func (p NamedLogProducer) log(logLevel LogLevel, message string) {
	mutex.Lock()
	defaultLogConsumer.Log(logLevel, message, p.name)
	mutex.Unlock()
}
